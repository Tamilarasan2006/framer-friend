...
function buildCordovaApiCandidates() {
    ...
    pushCandidate('https://nature-market.onrender.com');
...
function resolveApiBase() {
    if (isMobileRuntime()) {
        ...
        return candidates[0] || 'https://nature-market.onrender.com';
    }
    ...
    return window.location.port === '3000' ? '' : 'https://nature-market.onrender.com';
}
...
async function readSeedJson(relativePath, fallbackValue) {
    const seedDb = window.FARMERSFRIEND_SEED_DB;
    if (seedDb && typeof seedDb === 'object') {
        const filename = String(relativePath || '').split('/').pop();
        const map = {
            'users.json': 'users',
            'crops.json': 'crops',
            'market_products.json': 'marketProducts',
            'fertilizers.json': 'fertilizers',
            'cattle_feeds.json': 'cattleFeeds',
            'cattle_products.json': 'cattleProducts',
            'profiles.json': 'profiles',
            'doctors.json': 'doctors',
            'cattle_diseases.json': 'cattleDiseases',
            'ai_knowledge.json': 'aiKnowledge'
        };
        const key = map[filename];
        if (key && seedDb[key] !== undefined) {
            return cloneData(seedDb[key]);
        }
    }
...
async function requestJson(url, options = {}, fallback = null, fallbackMessage = 'Request failed') {
    ...
    const isAuthEndpoint = /^\/api\/auth\//.test(requestPath);
    ...
    try {
        ...
        if (!res) {
            ...
            throw new Error(
                ...
                'Set API URL with localStorage key farmersfriend_api_base (example: https://nature-market.onrender.com).'
            );
        }
    }
    ...
    if (!res.ok) {
        ...
        if (isMobileRuntime() && /^https?:\/\//i.test(url)) {
            ...
            const shouldRetry =
                (!hasExplicitApiError && res.status >= 400) ||
                (!isAuthEndpoint && (res.status === 404 || res.status >= 500));
            ...
            if (pathPart && shouldRetry) {
                ...
                for (const nextBase of retries) {
                    try {
                        ...
                        // Preserve expected auth validation errors from reachable backends,
                        // but do not block offline fallback on backend-not-found/server errors.
                        if (
                            isAuthEndpoint &&
                            nextData &&
                            typeof nextData === 'object' &&
                            nextData.error &&
                            [400, 401, 403, 409].includes(nextRes.status)
                        ) {
                            setApiBase(nextBase);
        ...
        if (isMobileRuntime()) {
            const hasExplicitApiError = !!(data && typeof data === 'object' && data.error);
            const shouldFallbackAuth =
                isAuthEndpoint &&
                (res.status === 404 || res.status >= 500 || !hasExplicitApiError);

            if (!isAuthEndpoint || shouldFallbackAuth) {
                return await offlineRequest(url, options);
            }
        }
